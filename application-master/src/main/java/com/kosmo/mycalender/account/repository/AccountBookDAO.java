...
public class AccountBookDAO {
	...
	public List<AccountBookDTO> findAll(String userId) {
		StringBuilder sql = new StringBuilder();
		List<AccountBookDTO> accountList;
		sql.append("select IDX, DATE, CATEGORY, DESCRIPTION, AMOUNT ");
		sql.append(", case when type = 'INCOME' then '수입' when type = 'EXPENSE' then '지출' end as TYPE ");
		sql.append(", USERID, REGDATE from ACCOUNT_BOOK where 1=1 ");
		sql.append("and USERID = ? ");
		sql.append("order by DATE ");

		accountList = jdbcTemplate.query(sql.toString(), rowMapper, userId);
	...
	public boolean updateRecord(AccountBookDTO record) {
		StringBuilder sql = new StringBuilder();
		sql.append("update ACCOUNT_BOOK set ");
		sql.append("DATE = ?, CATEGORY = ?, DESCRIPTION = ?, AMOUNT = ?, TYPE = ? ");
	...
	public AccountBookDTO findByIdx(Long idx) {
		StringBuilder sql = new StringBuilder();
		sql.append("select IDX, DATE, CATEGORY, DESCRIPTION, AMOUNT ");
		sql.append(", TYPE, USERID, REGDATE from ACCOUNT_BOOK where 1=1 ");
		sql.append(" and IDX = ?");
		
		List<AccountBookDTO> accountList = jdbcTemplate.query(sql.toString(), rowMapper, idx);
