	...
	function loadTotalbyDate(date){
		$.ajax({
			...
			success: function(totalRecords) {
				let html = ``;
				let count = 0;
				totalRecords.forEach(total => {
					html += `
						<tr>
							<td class="text-end mx-1" colspan="4">${total.type}</td>
							<td class="text-end mx-1" colspan="1">${total.count} 건</td>
							<td class="text-end mx-1" colspan="2">${new Intl.NumberFormat().format(total.totalAmount)}원</td>
						</tr>
					`;
					count = total.count;
				})
				$("#totalCount").text("총 " + count + " 건");
				$("#totalList").html(html);
			}
		})
	}
